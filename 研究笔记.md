...
这些仍然是 **GPT-OSS 形状的逻辑代理**：D/K 候选只匹配总量、激活比例和 top-N，不复制 DeepSeek 的 CSA/HCA 或 Kimi 的 Stable LatentMoE、KDA/MLA。除 O0 保留官方 131K 外，所有缩放候选统一记录 1,048,576 token 的目标上下文能力；该字段不参与参数账本，也不影响本文的缩放计算。
