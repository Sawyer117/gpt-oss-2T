...
| 缩放候选 | G3 / GPT-OSS 原生形状 / 只扩层反例 | 1.999T | 79.24B | 3.964% |
...
| G3 | 622 | 2,880 | 128 / 4 | 400K | Sliding + Full |
...
| G3 / GPT-OSS 原生形状 / 只扩层反例 | 8 | 8/16/32/64 | 99.7%（6×78 + 2×77层，不等层） | 99.7%（14×39 + 2×38层，不等层） | GPT-OSS 形状代理；尚无训练系统实测 |
...
| G3 / GPT-OSS 原生形状 / 只扩层反例 | 严格 8；KV复制 8/16/32 | 8/16/32/64 | 无静态形状约束；取决于 batch / SLO | PP8 99.7%（6×78 + 2×77层，不等层）；PP16 99.7%（14×39 + 2×38层，不等层） | GPT-OSS 形状代理；PP 对吞吐可行但会增加 decode stage 延迟 |
...
- **G3 / GPT 只扩层反例:** 总参数 1,999,006,227,136；激活参数 79,235,644,096。
...
G3 锁死 GPT-OSS-120B 的 hidden 2,880、expert intermediate 2,880、128 专家、top-4 和 attention 维度，只允许层数变化。此时参数账本退化为：

$$
P(L)=1{,}158{,}269{,}760+3{,}211{,}974{,}208L
$$

令 `P(L)=2T` 得到 `L=622.309`；为了保持 alternating sliding/full attention 的偶数层结构，取 **622 层**，对应总参数 **1.999T**、激活参数 **79.24B**。从 120B 的 36 层增至 622 层，顺序深度扩大 **17.28 倍**。

这不是数学上无法构造，而是不能作为合理的工业缩放方向：TP/EP 不能并行掉层间依赖；PP 虽能分摊权重和提高多 micro-batch 吞吐，却不能消除单个 token 必须依次穿过 622 个 Transformer block 的关键路径，还会增加 stage 边界通信。训练侧也将面对极端的激活重计算、pipeline 调度和深层残差优化压力。因此 G3 专门作为“总参数能对上，但架构明显不合理”的深度单轴反例。

可复算结果见 [生成候选表](research/model-scaling/candidates.generated.md)。主表以有研究价值的候选为主，同时重新纳入 G3 作为显式深度负对照；其他纯凑参数的单轴极端形状仍不保留：
...
- **G3 GPT-OSS 原生形状 / 只扩层反例:** 其他维度保持 120B，层数从 36 增至 622。它只用于证明单纯堆叠顺序深度虽然能凑到 2T，却不是合理部署候选。
...
1. **Rubin prefill + LPX decode:** 更偏好 active 参数较低、decode GEMM 规则的候选，L1 是计算侧下界，G3 则是顺序延迟的反例上界。但任何约 2T 权重都不可能整体常驻单个 128GB LPX，必须明确跨 rack 权重/专家放置与传输边界。
...
| 缩放候选 | G3 / GPT-OSS 原生形状 / 只扩层反例 | 1.999T | 79.24B | 3.964% | 622 | 2,880 | 128 / 4 | 400K | Sliding + Full | 仅层数 36→622，其他保持 120B |
