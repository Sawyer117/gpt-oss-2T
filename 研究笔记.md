...
这些仍然是 **GPT-OSS 形状的逻辑代理**：D/K 候选只匹配总量、激活比例和 top-N，不复制 DeepSeek 的 CSA/HCA 或 Kimi 的 Stable LatentMoE、KDA/MLA。所有缩放候选的上下文字段统一记录 NVIDIA Figure 11 的 400K 工作负载；该字段不参与参数账本，也不影响本文的缩放计算。D0/K0 的 1M 与 O0 的 131K 则保留各公开模型的官方能力值。
