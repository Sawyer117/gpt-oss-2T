...
| G3 | GPT-OSS 原生形状 / 只扩层反例 | GPT-OSS-120B 深度单轴反例 | 1.999T | 79.24B | 3.964% | -0.05% | 87.31B | -9.24% |
| F1 | GPT-OSS 族谱 / 20B→120B 线性外推 | GPT-OSS 20B→120B 族谱 | 1.997T | 14.68B | 0.735% | -0.15% | — | — |
...
- **G3 — GPT-OSS 原生形状 / 只扩层反例:** 只把 120B 的 36 层增至 622 层即可达到约 2T，但顺序深度放大 17.28 倍；PP 能分摊存储和吞吐，不能消除每 token 穿过 622 个 block 的依赖链，因此仅作为深度单轴反例。
- **F1 — GPT-OSS 族谱 / 20B→120B 线性外推:** 保持 GPT-OSS family 已观测的不变轴，并沿 20B→120B 的层数/专家数直线求解 2T；得到约 14.68B 激活，仅作为原生族谱外推锚点，EP 只能严格整分到 8。
